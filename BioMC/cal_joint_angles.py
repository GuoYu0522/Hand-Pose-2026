...
def calculate_joint_angle(joint, vis=False):
    '''
    :param joint: 21*3
    :param vis:
    :return: 15*2
    '''
    ALL_bones = np.array([
        joint[i] - joint[cfg_bmc.SNAP_PARENT[i]]
        for i in range(1, 21)
    ])
    ROOT_bones = ALL_bones[cfg_bmc.ID_ROOT_bone]  # FROM THUMB TO LITTLE FINGER
    ...
    if vis:
        ...
        parent = np.array(cfg_bmc.SNAP_PARENT[1:])
        ...
        ALL_joints_ID = np.array([cfg_bmc.ID_PIP_bone, cfg_bmc.ID_DIP_bone, cfg_bmc.ID_TIP_bone]).flatten()
