...
from BioMC import config_bmc_loss as cfg_bmc
...
def calculate_joint_angle_loss(thetas, hulls):
    ...
    for i in range(15):
        ...
        if not torch.sum(is_outside):
            sub_loss = torch.zeros(1, device=device, dtype=dtype)
        else:
...
def angle_between(v1, v2):
    epsilon = 1e-7
    cos = F.cosine_similarity(v1, v2, dim=-1).clamp(-1 + epsilon, 1 - epsilon)  # (B)
...
def interval_loss(value, min, max):
    ...
    zero = torch.zeros(1, device=value.device, dtype=value.dtype)

    loss1 = torch.max(min - value, zero)
    loss2 = torch.max(value - max, zero)
...
class BMCLoss:
    def __init__(
        ...
        self.lambda_ja = lambda_ja

        self.device = device if device is not None else torch.device(
            "cuda" if torch.cuda.is_available() else "cpu"
        )

        self.bmc_dir = bmc_dir

        self.bone_len_max = np.load(os.path.join(self.bmc_dir, "bone_len_max.npy"))
        self.bone_len_min = np.load(os.path.join(self.bmc_dir, "bone_len_min.npy"))
        self.rb_curvatures_max = np.load(os.path.join(self.bmc_dir, "curvatures_max.npy"))
        self.rb_curvatures_min = np.load(os.path.join(self.bmc_dir, "curvatures_min.npy"))
        self.rb_PHI_max = np.load(os.path.join(self.bmc_dir, "PHI_max.npy"))
        self.rb_PHI_min = np.load(os.path.join(self.bmc_dir, "PHI_min.npy"))

        self.joint_angle_limit = np.load(os.path.join(self.bmc_dir, "CONVEX_HULLS.npy"),
                                         allow_pickle=True)
        LEN_joint_angle_limit = len(self.joint_angle_limit)

        self.bl_max = torch.from_numpy(self.bone_len_max).float().to(self.device)
        self.bl_min = torch.from_numpy(self.bone_len_min).float().to(self.device)
        self.rb_curvatures_max = torch.from_numpy(self.rb_curvatures_max).float().to(self.device)
        self.rb_curvatures_min = torch.from_numpy(self.rb_curvatures_min).float().to(self.device)
        self.rb_PHI_max = torch.from_numpy(self.rb_PHI_max).float().to(self.device)
        self.rb_PHI_min = torch.from_numpy(self.rb_PHI_min).float().to(self.device)

        self.joint_angle_limit = [torch.from_numpy(self.joint_angle_limit[i]).float().to(self.device) for i in
    ...
    def compute_loss(self, joints):
        ...
        BMC_losses = {
            "bmc_bl": torch.zeros(1, device=device, dtype=dtype),
            "bmc_rb": torch.zeros(1, device=device, dtype=dtype),
            "bmc_ja": torch.zeros(1, device=device, dtype=dtype),
            }

        if (self.lambda_bl < 1e-6) and (self.lambda_rb < 1e-6) and (self.lambda_ja < 1e-6):
            return final_loss, BMC_losses

        ALL_bones = [
            (
                    joints[:, i, :] -
                    joints[:, cfg_bmc.SNAP_PARENT[i], :]
            ) for i in range(21)
        ]
        ALL_bones = torch.stack(ALL_bones[1:], dim=1)  # (B,20,3)
        ROOT_bones = ALL_bones[:, cfg_bmc.ID_ROOT_bone]  # (B,5,3)
        PIP_bones = ALL_bones[:, cfg_bmc.ID_PIP_bone]
        DIP_bones = ALL_bones[:, cfg_bmc.ID_DIP_bone]
        TIP_bones = ALL_bones[:, cfg_bmc.ID_TIP_bone]

        ALL_Z_axis = normalize(ALL_bones)
        PIP_Z_axis = ALL_Z_axis[:, cfg_bmc.ID_ROOT_bone]
        DIP_Z_axis = ALL_Z_axis[:, cfg_bmc.ID_PIP_bone]
        TIP_Z_axis = ALL_Z_axis[:, cfg_bmc.ID_DIP_bone]

        normals = normalize(cross_product(ROOT_bones[:, 1:], ROOT_bones[:, :-1]))

        # compute loss of bone length
        bl_loss = torch.zeros(1, device=device, dtype=dtype)
        if self.lambda_bl:
        ...
        rb_loss = torch.zeros(1, device=device, dtype=dtype)
        if self.lambda_rb:
        ...
        ja_loss = torch.zeros(1, device=device, dtype=dtype)
        if self.lambda_ja:
            # PIP bones
            PIP_X_axis = torch.zeros((batch_size, 5, 3), device=device, dtype=dtype)  # (B,5,3)
            ...
            ja_loss = calculate_joint_angle_loss(ALL_theta, self.joint_angle_limit)
            final_loss += self.lambda_ja * ja_loss

        BMC_losses["bmc_ja"] = ja_loss
